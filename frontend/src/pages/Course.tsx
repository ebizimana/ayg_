...
export default function CoursePage() {
    ...
    try {
      ...
      const mappedAssignments: Assignment[] = assignmentsByCategory
        ...
        .map((a: any) => ({
          ...
          expected: a.grade?.expectedPoints ?? undefined,
        }));
  ...
  const pointsLeftToLose = useMemo(() => {
    ...
    const remainingMax = assignments.reduce((sum, a) => sum + (a.max - (a.earned ?? 0)), 0);
    const left = earnedPoints + remainingMax - targetPoints;
    return Math.max(Math.round(left), 0);
  }, [assignments, earnedPoints, selectedGrade, totalMax]);

  const statusCopy = pointsLeftToLose > 0 ? "On Track" : "Needs Attention";
  ...
  const saveAssignment = async () => {
    ...
    const hasGradeInput = payload.earned !== undefined || payload.expected !== undefined;
    try {
      if (mode === "add") {
        ...
        if (hasGradeInput) {
          await upsertGrade(created.id, payload.earned, payload.expected);
        }
        setAssignments((prev) => [
          ...prev,
          {
        ...
        if (hasGradeInput) {
          await upsertGrade(payload.id, payload.earned, payload.expected);
        }
        setAssignments((prev) =>
          prev.map((a) =>
            a.id === payload.id
              ? { ...a, name: payload.name, max: payload.max, earned: payload.earned, expected: payload.expected }
              : a,
          ),
        );
      }
  ...
  const upsertGrade = async (assignmentId: string, earned?: number, expected?: number) => {
    try {
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 space-y-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 rounded-2xl border border-slate-200 bg-white/80 shadow-md backdrop-blur">
          <div className="flex items-center gap-4 px-5 py-4">
            <div className="h-12 w-12 rounded-xl bg-gradient-to-br from-primary to-primary-dark text-white font-bold grid place-items-center shadow-lg">
              {heroLetter}
            </div>
            <div>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          ...
          <CardContent className="flex items-center gap-3">
            <Badge className="rounded-full px-3 py-1 text-lg font-bold">{selectedGrade}</Badge>
            <div>
              <p className="text-3xl font-bold text-foreground">
                {actualPercent !== null ? Math.round(actualPercent * 100) : currentPercent}%
              </p>
              <p className="text-sm text-muted-foreground">
                Based on {gradedCount} graded {gradedCount === 1 ? "assignment" : "assignments"}.
              </p>
            </div>
          </CardContent>
          </Card>
          ...
          <Card className="border-slate-200 shadow-md">
            ...
            <CardContent className="space-y-2">
              <p className="text-3xl font-bold text-foreground">{pointsLeftToLose} pts</p>
              <p className="text-sm text-muted-foreground">
                Total points you can still lose and keep your target grade.
              </p>
              {lastRun && <p className="text-xs text-success-foreground bg-success/10 rounded-md px-3 py-2">{lastRun}</p>}
            </CardContent>
          </Card>
          ...
          <Card className="border-slate-200 shadow-md">
            ...
            <CardContent className="space-y-3">
              ...
              <p className="text-sm text-muted-foreground">
                Run simulations and update grades to stay on track. Keep adjusting target grades to explore scenarios.
              </p>
